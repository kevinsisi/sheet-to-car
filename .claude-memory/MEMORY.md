...
| Chat routing hardening | The chat agent now deterministically routes clear direct-generation, short command forms like `出 FB 文案`, owner-check, readiness intents, and explicit multi-platform direct-generation commands for supported phrases, while avoiding unsupported platforms, extra one-off copy constraints, and broad follow-up confirmations |
